...
<style>
  ...
  .pagehd__controls { display:flex; align-items:flex-end; gap:18px; flex-wrap:wrap; }
  .picker { display:flex; flex-direction:column; gap:6px; }
  .bulk-controls { display:flex; flex-direction:column; gap:6px; align-items:flex-start; }
  .bulk-templink { font-family:var(--font-mono); font-size:11px; color:var(--g-blue-l); text-decoration:underline; }
  .bulk-templink:hover { color:var(--g-blue); }
...
<main>
  <div class="wrap">
    ...
    <div class="pagehd">
      ...
      <div class="pagehd__controls">
        <div class="picker">
          <span class="picker__label">Workspace</span>
          <select id="wsPicker" aria-label="Select workspace"></select>
        </div>
        <div class="bulk-controls">
          <button type="button" class="gc-btn gc-btn--ghost gc-btn--sm" id="bulkUploadBtn">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><path d="M17 8l-5-5-5 5"/><path d="M12 3v12"/></svg>
            Bulk upload
          </button>
          <a class="bulk-templink" href="/api/use-cases/template.csv" download>Download template CSV</a>
        </div>
      </div>
    </div>
...
<script src="assets/bulk-upload.js"></script>
<script>
(function () {
  "use strict";
  ...
  // ---------- bulk upload modal ----------
  // Reusable feature module (assets/bulk-upload.js). Defaults its workspace
  // picker to whatever's currently selected, and reloads the portfolio when
  // any rows are inserted so the new use cases show up immediately.
  if (window.GAIC_BULK) {
    window.GAIC_BULK.mount({
      open: '#bulkUploadBtn',
      workspaceId: function () { return el.wsPicker.value || (STATE.selectedWs ? STATE.selectedWs.id : null); },
      onInserted: function () {
        selectAndLoad(el.wsPicker.value || (STATE.selectedWs ? STATE.selectedWs.id : null));
      },
    });
  }
